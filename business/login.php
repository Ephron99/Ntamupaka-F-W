...
if(isset($db,$_POST['submit'])){
    ...
    $query1=mysqli_query($db,"SELECT * FROM business");
	while($row=mysqli_fetch_array($query1))
	{
        $db_id=$row["id"];
        $db_name=$row["name"];
		$db_city=$row["city"];
		$db_username=$row["TINnumber"];
		...
		if($username==$db_username && $password==$db_password){
			...
			if($_SESSION["status"]=='approve'){
				...
				header("Location:sys/dashboard.php?search=");
			}
			if($_SESSION["status"]=='panding'){
               
				header("Location:sys/dashboard2.php");
			}
		}
			...
			<div class="wrap-login100 p-t-30 p-b-50">
				...
				<form class="login100-form validate-form p-b-33 p-t-5" method="post" action="login.php">
					...
					<div class="wrap-input100 validate-input" data-validate = "Enter username">
						<input class="input100" type="text" name="username" placeholder="TIN Number">
					...
					<div class="container-login100-form-btn m-t-32">
						<a class="nav-link" href="signup.php" class="" >
